...
==> UPDATE 쿼리 1 실행할 때 안쪽 SELECT 쿼리가 2개가 포함됨 => 비효율적
    고정된 값으로 업데이트하는 것이 아니라 다른 테이블에 있는 값을 통해서 업데이트 할 때
    비효율이 존재
...
99번 부서의 부서명, loc정보를 유지하고 싶음,
다른 트랜잭션에서 수정하지 못하도록 막고 싶음
...
PHANTOM READ :
LV2에서는 테이블에 존재하는 데이터에 대해 후행 트랜잭션에서 작업하지 못하도록 막을 수는 있지만
후행 트랜잭션에서 신규로 입력하는 데이터는 막을 수 없다.
즉 선행 트랜잭션에서 처음 읽은 데이터와 후행트랜잭션에서 신규입력후 커밋한 이후에 조회한 데이터가
불일치할 수 있다 (없던 데이터가 갑자기 생성되는 현상)
...
후행 트랜잭션에서 dept테이블에 데이터 입력후 커밋
SELECT *
FROM dept;
ROLLBACK;
--팬텀리드현상이 없어짐

DML (Data Manipulation(조작) LANGUAGE) : 데이터를 다루는(조작하는) SQL
...
ex : 1. 사원 테이블에 중복된 사원번호가 등록되는 것을 방지
     2. 반드시 입력이 되어야 하는 컬럼의 값을 확인
==> 파일시스템이 가질 수 없는 성격

오라클에서 제공하는 데이터 무결성을 지키기 위해 제공하는 제약조건 5가지(4가지)
1. NOT NULL
   해당 컬럼에 값이 NULL으로 들어오는 것을 제약, 방지
   (ex. emp 테이블의 empno 컬럼)
   
2. UNIQUE
   전체 행 중에 해당 컬럼의 값이 중복이 되면 안된다.
